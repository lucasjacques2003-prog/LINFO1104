...
define
    ...
    fun {ValidateTransaction Transaction State}
        ...
        elseif (Transaction.value + Transaction.effort) > State.(Transaction.sender).balance then false
    ...
    fun {ApplyTransaction Transaction State}
        NewSender = user(balance: State.(Transaction.sender).balance - (Transaction.value + Transaction.effort) nonce: State.(Transaction.sender).nonce +1)
